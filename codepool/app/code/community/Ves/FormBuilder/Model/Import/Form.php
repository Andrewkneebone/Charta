 ...
 */
class Ves_FormBuilder_Model_Import_Form extends Mage_Core_Model_Abstract {
			...
			while (($data = fgetcsv($handle, 110000, $this->delimiter)) !== FALSE) {
				...
				foreach ($this->header_columns as $index => $keyname) {
					...
					switch ($keyname) {
						...
						$table = $resource->getTableName('ves_formbuilder/form');
				...
				try {
					$model->save();
					Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('ves_formbuilder')->__('Updated ' . $identifier));
				} catch (Exception $e) {
